...
export default function NotasIAPage() {
  ...
  const deleteNote = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/saved-notes?id=${id}`, { method: "DELETE" });
      if (res.ok) {
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="relative w-full sm:w-64">
          <input
            ...
            className="w-full pl-9 pr-4 py-2 bg-card border border-border rounded-lg text-sm focus:ring-2 focus:ring-blue-500/50 outline-none transition-all"
          />
      ...
      {isLoading ? (
        ...
        <div className="bg-card border border-border rounded-xl p-12 text-center flex flex-col items-center justify-center">
          <Bookmark size={48} className="text-foreground-muted mb-4 opacity-50" />
          <h3 className="text-lg font-medium text-foreground mb-2">No hay notas guardadas</h3>
          <p className="text-foreground-muted text-sm max-w-sm">
            Podés guardar notas haciendo clic en el botón "Guardar" debajo de las respuestas del asistente ClinIA.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
          {filteredNotes.map(note => (
            <div key={note.id} className="bg-card border border-border rounded-xl shadow-sm hover:shadow-md transition-shadow overflow-hidden flex flex-col h-[350px]">
              <div className="p-4 border-b border-border bg-muted/20 flex justify-between items-start shrink-0">
                <div>
                  ...
                  <p className="text-xs text-foreground-muted mt-1">{new Date(note.createdAt).toLocaleDateString()}</p>
                </div>
                <div className="flex items-center gap-1">
                  <button onClick={() => exportNote(note)} className="p-1.5 text-foreground-muted hover:text-blue-600 hover:bg-blue-50 rounded-md transition-colors" title="Exportar">
                    <Download size={16} />
                  </button>
                  <button onClick={() => deleteNote(note.id)} className="p-1.5 text-foreground-muted hover:text-red-600 hover:bg-red-50 rounded-md transition-colors" title="Eliminar">
                    <Trash2 size={16} />
                  </button>
                </div>
              </div>
              ...
              <div className="p-4 overflow-y-auto flex-1 prose prose-sm dark:prose-invert max-w-none 
