...
const EditarPromocionPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if ((type === 'BUY_X_GET_Y' || type === 'SET_DISCOUNT') && conditions.length === 0) {
      setError('Agregá al menos una condición.');
      return;
    }
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <h1 className="text-2xl font-bold text-foreground mb-6">Editar Promoción</h1>
      ...
      <form onSubmit={handleSubmit} className="bg-muted p-6 rounded-xl shadow-lg space-y-4">
        ...
        {type === 'THRESHOLD' && (
          <Input label="Umbral mínimo ($)" type="number" step="0.01" min="0" value={minQuantity}
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <Input label="Valor" type="number" step="0.01" min="0" value={discountValue} onChange={e => setDiscountValue(e.target.value)} required />
        </div>

        {type === 'BUY_X_GET_Y' && (
          <Input label="Máx. unidades con descuento" type="number" min="1" value={maxDiscountQty} onChange={e => setMaxDiscountQty(e.target.value)} />
        )}

        <Input label="Prioridad" type="number" value={priority} onChange={e => setPriority(e.target.value)} />

        <div className="grid grid-cols-2 gap-4">
          <Input label="Válido desde" type="date" value={startDate} onChange={e => setStartDate(e.target.value)} />
          <Input label="Válido hasta" type="date" value={endDate} onChange={e => setEndDate(e.target.value)} />
        </div>

        <div>
          <label className="block text-sm font-medium text-foreground mb-1">Condiciones</label>
            ...
            <div className="mb-3 border border-border rounded-md bg-background max-h-40 overflow-y-auto">
              {searchResults.map((item: any) => (
                <button key={item.id} type="button" onClick={() => addCondition(item)}
                  className="w-full text-left px-3 py-2 text-sm hover:bg-accent transition-colors">{item.name}</button>
              ))}
            </div>
          )}
          ...
          <div className="space-y-2">
            {conditions.map((c, i) => (
              <div key={i} className="flex items-center gap-2 bg-background p-2 rounded-md border border-border">
                ...
                <input type="number" min="1" value={c.minQuantity} onChange={e => updateConditionQty(i, e.target.value)}
...
export default EditarPromocionPage;
