  ...
  return (
    <div className="mb-4 p-3.5 bg-blue-50 border border-blue-200 rounded-xl flex flex-wrap items-center justify-between gap-3 shadow-xs">
      <div className="flex flex-wrap items-center gap-2 text-sm text-blue-950">
        ...
        {!isAllPagesSelected && totalCount && totalCount > count && onSelectAllPages && (
          <button
            type="button"
          ...
          >
            Seleccionar los {totalCount.toLocaleString("es-AR")} productos en total 🚀
          </button>
        )}
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        {onPublishWeb && (
          <Button variant="outline" size="sm" onClick={onPublishWeb} className="border-emerald-600 text-emerald-700 hover:bg-emerald-100 bg-white font-semibold">
            🌐 Publicar en Web
          </Button>
        )}
        {onHideWeb && (
          <Button variant="outline" size="sm" onClick={onHideWeb} className="border-gray-400 text-gray-700 hover:bg-gray-100 bg-white font-semibold">
            🚫 Ocultar de Web
          </Button>
        )}
        <Button variant="outline" size="sm" onClick={onClear} className="bg-white">
