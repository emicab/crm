...
const AdditionalDetailsSection: React.FC<AdditionalDetailsSectionProps> = ({
  ...
  const showClientes = isModuleEnabled("clientes");
  const showVendedores = isModuleEnabled("vendedores");
  const showDescuentos = isModuleEnabled("combos_promociones");

  // Si todos los campos superiores están ocultos, cambia la grilla
  const hasUpperFields = showClientes || showVendedores || showDescuentos;
  ...
  return (
    <details className="border border-border rounded-md p-4">
      <summary className="text-sm font-medium text-primary cursor-pointer select-none">
        Detalles adicionales {showClientes && " (cliente)"} {showVendedores && " (vendedor)"} {showDescuentos && " (descuento)"}
      </summary>
